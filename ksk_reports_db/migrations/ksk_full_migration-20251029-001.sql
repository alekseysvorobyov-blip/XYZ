...
-- ============================================================================
-- Дата создания: 2025-10-29 12:40:35
...
-- ============================================================================
-- ФАЙЛ: put_ksk_result_v3.sql
-- Путь: D:\Yandex.Drive\Disk\YandexDisk\Документы\КСК\upoa_ksk_reports\deepseek\ksk_reports_db\schema\002_functions\core\put_ksk_result_v3.sql
-- Размер: 14.51 KB
-- ============================================================================
...
--   p_input_kafka_partition  - Номер партиции Kafka входящего сообщения (ДОБАВЛЕНО 29.10.2025)
--   p_input_kafka_offset     - Offset сообщения в партиции Kafka (ДОБАВЛЕНО 29.10.2025)
--   p_input_kafka_headers    - JSONB заголовки входящего сообщения из Kafka (ДОБАВЛЕНО 29.10.2025)
--   p_output_kafka_headers   - JSONB заголовки исходящего сообщения из Kafka (ДОБАВЛЕНО 29.10.2025)
--
-- ВОЗВРАЩАЕТ:
...
--   2025-10-29 - Добавлены Kafka параметры для отслеживания источников сообщений
--              - Добавлены параметры для хранения заголовков Kafka сообщений
--   2025-10-29 02:49 - Применены критические правки NULL-безопасности
--              - Защита v_search_results от NULL
--              - Защита CTE list_codes от NULL результата
--              - Замена jsonb_typeof на jsonb_array_length для match
--              - Защита string_agg от NULL документов
...
CREATE OR REPLACE FUNCTION upoa_ksk_reports.put_ksk_result(
    ...
    p_input_json       JSONB,
    p_output_json      JSONB,
    p_input_kafka_partition INTEGER DEFAULT NULL,    -- ДОБАВЛЕНО 29.10.2025
    p_input_kafka_offset BIGINT DEFAULT NULL,         -- ДОБАВЛЕНО 29.10.2025
    p_input_kafka_headers    JSONB DEFAULT NULL,      -- ДОБАВЛЕНО 29.10.2025
    p_output_kafka_headers   JSONB DEFAULT NULL       -- ДОБАВЛЕНО 29.10.2025
)
RETURNS INTEGER
LANGUAGE plpgsql
AS $function$
...
BEGIN
    -- =========================================================================
    -- ВАЛИДАЦИЯ ВХОДНЫХ ПАРАМЕТРОВ (ДОБАВЛЕНО 29.10.2025 02:49)
    -- =========================================================================
    IF p_input_timestamp IS NULL THEN
        RAISE EXCEPTION 'p_input_timestamp cannot be NULL';
    END IF;
    
    IF p_output_timestamp IS NULL THEN
        RAISE EXCEPTION 'p_output_timestamp cannot be NULL';
    END IF;
    
    IF p_input_json IS NULL THEN
        RAISE EXCEPTION 'p_input_json cannot be NULL (use empty JSON {})';
    END IF;
    
    IF p_output_json IS NULL THEN
        RAISE EXCEPTION 'p_output_json cannot be NULL (use empty JSON {})';
    END IF;
    
    IF p_input_kafka_partition IS NULL THEN
        RAISE EXCEPTION 'p_input_kafka_partition cannot be NULL (use -1 for unknown)';
    END IF;
    
    IF p_input_kafka_offset IS NULL THEN
        RAISE EXCEPTION 'p_input_kafka_offset cannot be NULL (use -1 for unknown)';
    END IF;
    ...
    v_search_results     := COALESCE(p_output_json->'searchCheckResultKCKH', '[]'::jsonb);  -- ИСПРАВЛЕНО 29.10.2025 02:49
    ...
    WITH list_codes_cte AS (
        ...
        SELECT COALESCE(array_agg(DISTINCT elem->>'listCode'), '{}'::TEXT[]) AS codes  -- ИСПРАВЛЕНО 29.10.2025 02:49
    ...
    INSERT INTO upoa_ksk_reports.ksk_result (
        ...
        -- Kafka метаданные (ДОБАВЛЕНО 29.10.2025)
    ...
    SELECT
        ...
        lc.codes,  -- Уже защищено COALESCE в CTE (ИСПРАВЛЕНО 29.10.2025 02:49)
        ...
        COALESCE(
            (SELECT string_agg(elem->>'documentType', ';') 
             FROM jsonb_array_elements(COALESCE(v_payer_info->'documents', '[]'::jsonb)) AS elem  -- ИСПРАВЛЕНО 29.10.2025 02:49
             WHERE elem->>'documentType' IS NOT NULL),  -- ИСПРАВЛЕНО 29.10.2025 02:49
            ''
        ),
        ...
        COALESCE(
            (SELECT string_agg(elem->>'documentType', ';') 
             FROM jsonb_array_elements(COALESCE(v_receiver_info->'documents', '[]'::jsonb)) AS elem  -- ИСПРАВЛЕНО 29.10.2025 02:49
             WHERE elem->>'documentType' IS NOT NULL),  -- ИСПРАВЛЕНО 29.10.2025 02:49
            ''
        ),
        -- Kafka метаданные (ДОБАВЛЕНО 29.10.2025)
    ...
    LOOP
        ...
        IF jsonb_array_length(v_figurant_record.figurant_data->'match') > 0 THEN  -- ИСПРАВЛЕНО 29.10.2025 02:49
...
-- ============================================================================
-- Всего файлов обработано: 36
-- Дата завершения: 2025-10-29 12:40:35
-- ============================================================================
