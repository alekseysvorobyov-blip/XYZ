...
-- ============================================================================
-- Дата создания: 2025-10-29 15:33:30
...
-- ============================================================================
-- ФАЙЛ: put_ksk_result_ai_generated_20251029_005.sql
-- ============================================================================
-- ОПИСАНИЕ:
...
--
-- ДАТА СОЗДАНИЯ: 29.10.2025 14:31 MSK
-- ВЕРСИЯ: 3.4
--
...
--   + RETURN -1 вместо RAISE → транзакция НЕ откатывается
--
-- ЛОГИКА ВАЛИДАЦИИ:
--   1. Проверяем все параметры через IF-ELSIF
--   2. Если хоть один NULL → сохраняем в v_validation_error
--   3. Если v_validation_error NOT NULL → ОДИН INSERT + RETURN -1
...
--   ✅ Приложение получает -1 и увеличивает errorCounter
--   ✅ Batch продолжается для других записей
--
-- ИНТЕГРАЦИЯ С JAVA SPRING:
--   Integer result = jdbcTemplate.queryForObject(...);
--   if (result == -1) errorCounter.increment();
--
-- ПАТТЕРНЫ ВЗЯТЫ ИЗ:
...
BEGIN
    ...
    -- Если есть ошибка валидации - логируем ОДИН РАЗ и выходим
...
EXCEPTION
    WHEN OTHERS THEN
        -- Обработка runtime ошибок (SQL, JSON parsing, constraints)
        INSERT INTO upoa_ksk_reports.ksk_result_error (
            error_code,
            error_message,
            input_timestamp,
...
Версия: 3.4 от 29.10.2025

ВОЗВРАЩАЕМЫЕ ЗНАЧЕНИЯ:
  > 0  - ID вставленной записи (успех)
  = -1 - Ошибка валидации или runtime (детали в ksk_result_error)
...
ОБРАБОТКА ОШИБОК:
  - Валидация: error_code = PARAM_NULL
  - Runtime: error_code = SQLSTATE
...
-- Дата завершения: 2025-10-29 15:33:30
-- ============================================================================
