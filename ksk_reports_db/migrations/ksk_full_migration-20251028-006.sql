...
-- ============================================================================
-- Дата создания: 2025-10-27 23:30:55
...
-- ============================================================================
-- ФАЙЛ: add_column_if_not_exists.sql
-- Путь: D:\Yandex.Drive\Disk\YandexDisk\Документы\КСК\upoa_ksk_reports\deepseek\ksk_reports_db\schema\000_core\add_column_if_not_exists.sql
-- Размер: 4.82 KB
-- ============================================================================
...
-- ============================================================================
-- ФАЙЛ: jsonb_object_length.sql
-- Путь: D:\Yandex.Drive\Disk\YandexDisk\Документы\КСК\upoa_ksk_reports\deepseek\ksk_reports_db\schema\000_core\jsonb_object_length.sql
-- Размер: 2.08 KB
-- ============================================================================
...
RAISE NOTICE '[ksk_result] ✅ Проверка и добавление колонок завершена';
...
RAISE NOTICE '[ksk_result] ✅ Индексы созданы/проверены';
...
RAISE NOTICE '[ksk_figurant] ✅ Проверка и добавление колонок завершена';
...
RAISE NOTICE '[ksk_figurant] ✅ Индексы созданы/проверены';
...
RAISE NOTICE '[ksk_figurant_match] ✅ Проверка и добавление колонок завершена';
...
RAISE NOTICE '[ksk_figurant_match] ✅ Индексы созданы/проверены';
...
SELECT upoa_ksk_reports.add_column_if_not_exists('upoa_ksk_reports.ksk_system_operations_log', 'begin_time', 'TIMESTAMP', 'now()');
SELECT upoa_ksk_reports.add_column_if_not_exists('upoa_ksk_reports.ksk_system_operations_log', 'end_time', 'TIMESTAMP');
...
RAISE NOTICE '[ksk_system_operations_log] ✅ Проверка и добавление колонок завершена';
...
RAISE NOTICE '[ksk_system_operations_log] ✅ Индексы созданы/проверены';
...
RAISE NOTICE '[ksk_report_orchestrator] ✅ Проверка и добавление колонок завершена';
...
DECLARE
    v_index_name text;
    v_needed_indexes text[] := ARRAY[
        'idx_ksk_report_orchestrator_code'
    ];
    v_index_count integer := 0;
BEGIN
    FOR v_index_name IN
    ...
    LOOP
        IF NOT v_index_name = ANY(v_needed_indexes) THEN
            EXECUTE 'DROP INDEX IF EXISTS upoa_ksk_reports.' || quote_ident(v_index_name);
            RAISE NOTICE '[ksk_report_orchestrator] 🗑️  Удалён ненужный индекс: %', v_index_name;
            v_index_count := v_index_count + 1;
        END IF;
    END LOOP;
    
    IF v_index_count = 0 THEN
        RAISE NOTICE '[ksk_report_orchestrator] ℹ️  Ненужных индексов не найдено';
    ELSE
        RAISE NOTICE '[ksk_report_orchestrator] ✅ Удалено % ненужных индексов', v_index_count;
    END IF;
END $$;
...
-- 4.1. B-tree индекс на report_code
-- Применение: поиск типа отчёта по коду (WHERE report_code = 'totals')
-- Используется при генерации и управлении отчётами
--
CREATE INDEX IF NOT EXISTS idx_ksk_report_orchestrator_code
  ON upoa_ksk_reports.ksk_report_orchestrator (report_code);
COMMENT ON INDEX upoa_ksk_reports.idx_ksk_report_orchestrator_code 
  IS 'B-tree: Поиск типа отчёта по коду. Основной индекс для управления отчётами.';

RAISE NOTICE '[ksk_report_orchestrator] ✅ Индексы созданы/проверены';

-- ============================================================================
-- 5. ИНИЦИАЛИЗАЦИЯ ОРКЕСТРАТОРА (идемпотентно)
-- ============================================================================
...
RAISE NOTICE '[ksk_report_orchestrator] ✅ Оркестратор инициализирован (5 типов отчётов)';
...
RAISE NOTICE '[ksk_report_header] ✅ Проверка и добавление колонок завершена';
...
RAISE NOTICE '[ksk_report_header] ✅ Индексы созданы/проверены';
...
RAISE NOTICE '[ksk_report_totals_data] ✅ Проверка и добавление колонок завершена';
...
RAISE NOTICE '[ksk_report_totals_data] ✅ Индексы созданы/проверены';
...
RAISE NOTICE '[ksk_report_list_totals_data] ✅ Проверка и добавление колонок завершена';
...
RAISE NOTICE '[ksk_report_list_totals_data] ✅ Индексы созданы/проверены';
...
RAISE NOTICE '[ksk_report_totals_by_payment_type_data] ✅ Проверка и добавление колонок завершена (44 колонки!)';
...
RAISE NOTICE '[ksk_report_totals_by_payment_type_data] ✅ Индексы созданы/проверены';
...
RAISE NOTICE '[ksk_report_list_totals_by_payment_type_data] ✅ Проверка и добавление колонок завершена (39 колонок!)';
...
RAISE NOTICE '[ksk_report_list_totals_by_payment_type_data] ✅ Индексы созданы/проверены';
...
RAISE NOTICE '[ksk_report_figurants_data] ✅ Проверка и добавление колонок завершена';
...
RAISE NOTICE '[ksk_report_figurants_data] ✅ Индексы созданы/проверены';
...
CREATE OR REPLACE FUNCTION upoa_ksk_reports.put_ksk_result(
    p_input_timestamp  TIMESTAMP WITHOUT TIME ZONE,
    p_output_timestamp TIMESTAMP WITHOUT TIME ZONE,
    p_input_json       JSONB,
    p_output_json      JSONB
)
RETURNS INTEGER
...
--     - Платёж: payment_id, payment_purpose, account_debet, account_сredit
...
-- ============================================================================
-- Всего файлов обработано: 30
-- Дата завершения: 2025-10-27 23:30:55
-- ============================================================================
