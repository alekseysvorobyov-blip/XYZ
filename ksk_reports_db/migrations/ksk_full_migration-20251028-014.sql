...
-- ============================================================================
-- Дата создания: 2025-10-28 18:25:16
...
-- - Kafka headers (input/output) -- ДОБАВЛЕНО 28.10.2025
...
-- ОПИСАНИЕ: Основная таблица с результатами проверки КСК
-- Дата: 2025-10-28 (обновлено: добавлены kafka headers)
-- ============================================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        CREATE TABLE upoa_ksk_reports.ksk_result (
            ...
            -- Kafka headers (ДОБАВЛЕНО 28.10.2025)
        ...
        COMMENT ON COLUMN upoa_ksk_reports.ksk_result.input_kafka_headers 
            IS 'Kafka headers от входящего сообщения (upoa_enriched_transactions) - EXTERNAL STORAGE';
        
        COMMENT ON COLUMN upoa_ksk_reports.ksk_result.output_kafka_headers 
            IS 'Kafka headers от выходящего сообщения (upoa_ksk_results) - EXTERNAL STORAGE';
...
DO $$
BEGIN
    -- Устанавливаем STORAGE EXTERNAL для новых колонок если таблица уже существовала
    BEGIN
        ALTER TABLE upoa_ksk_reports.ksk_result 
            ALTER COLUMN input_kafka_headers SET STORAGE EXTERNAL;
...
-- НАЗНАЧЕНИЕ: Вставка записи результата проверки КСК в таблицу ksk_result
-- ============================================================================
-- ОПИСАНИЕ:
--   Функция принимает на вход два JSONB объекта (input и output),
--   извлекает из них необходимые поля, и вставляет запись в партиционированную
--   таблицу ksk_result. После вставки основной записи, функция также обрабатывает
--   массив figurants из output_json и вставляет связанные записи в ksk_figurant.
--
-- ПАРАМЕТРЫ:
--   p_input_timestamp  - временная метка входящего сообщения
--   p_output_timestamp - временная метка выходящего сообщения (ключ партиционирования)
--   p_input_json       - JSONB объект с входящими данными
--   p_output_json      - JSONB объект с результатами проверки
--   p_input_kafka_headers  - JSONB с Kafka headers входящего сообщения (ДОБАВЛЕНО 28.10.2025)
--   p_output_kafka_headers - JSONB с Kafka headers выходящего сообщения (ДОБАВЛЕНО 28.10.2025)
--
-- ВОЗВРАЩАЕТ:
--   INTEGER - ID вставленной записи (ksk_result.id)
--
-- ИСПОЛЬЗОВАНИЕ:
--   SELECT put_ksk_result(
--     '2025-10-28 10:00:00'::timestamp,
--     '2025-10-28 10:00:01'::timestamp,
--     '{"corrId": "abc123", ...}'::jsonb,
--     '{"resolution": "allow", ...}'::jsonb,
--     '{"trace_id": "xyz", ...}'::jsonb,
--     '{"trace_id": "xyz", ...}'::jsonb
--   );
--
-- ИСТОРИЯ ИЗМЕНЕНИЙ:
--   2025-10-27 - Создание функции с оптимизацией и денормализацией
--   2025-10-28 - Добавлена поддержка Kafka headers (input/output)
-- ============================================================================
...
CREATE OR REPLACE FUNCTION upoa_ksk_reports.put_ksk_result(
    ...
    p_input_json JSONB,
    p_output_json JSONB,
    p_input_kafka_headers JSONB DEFAULT NULL,   -- ДОБАВЛЕНО 28.10.2025
    p_output_kafka_headers JSONB DEFAULT NULL   -- ДОБАВЛЕНО 28.10.2025
)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
    ...
    -- Переменные для извлечения из input_json
    ...
    -- Переменные для извлечения из output_json
    v_payment_type TEXT;
    v_resolution TEXT;
    v_list_codes TEXT[];
    v_has_bypass TEXT;
    
    -- Для обработки фигурантов
    v_figurants JSONB;
    v_figurant JSONB;
...
BEGIN
    -- ========================================================================
    -- 1. ИЗВЛЕЧЕНИЕ БАЗОВЫХ ПОЛЕЙ
    -- ========================================================================
    
    v_corr_id := p_input_json->>'corrId';
    v_date := p_output_timestamp::DATE;
    
    -- ========================================================================
    -- 2. ИЗВЛЕЧЕНИЕ ИЗ INPUT_JSON (ДЕНОРМАЛИЗАЦИЯ ДЛЯ ОПТИМИЗАЦИИ)
    -- ========================================================================
    
    v_payment_info := p_input_json->'paymentInfo';
    
    -- Базовая информация о платеже
    v_payment_id := v_payment_info->>'paymentId';
    v_payment_purpose := v_payment_info->>'paymentPurpose';
    v_account_debet := v_payment_info->>'accountDebet';
    v_account_credit := v_payment_info->>'accountCredit';
    
    -- Плательщик
    v_payer := v_payment_info->'payer';
    v_payer_inn := v_payer->>'inn';
    v_payer_name := v_payer->>'name';
    v_payer_account_number := v_payer->>'accountNumber';
    v_payer_document_type := (v_payer->'document'->>'type');
    
    v_payer_bank := v_payer->'bank';
    v_payer_bank_name := v_payer_bank->>'name';
    v_payer_bank_account_number := v_payer_bank->>'accountNumber';
    
    -- Получатель
    v_receiver := v_payment_info->'receiver';
    v_receiver_account_number := v_receiver->>'accountNumber';
    v_receiver_name := v_receiver->>'name';
    v_receiver_inn := v_receiver->>'inn';
    v_receiver_document_type := (v_receiver->'document'->>'type');
    
    v_receiver_bank := v_receiver->'bank';
    v_receiver_bank_name := v_receiver_bank->>'name';
    v_receiver_bank_account_number := v_receiver_bank->>'accountNumber';
    
    -- Финансовая информация
    v_amount := v_payment_info->>'amount';
    v_currency := v_payment_info->>'currency';
    v_currency_control := v_payment_info->>'currencyControl';
    
    -- ========================================================================
    -- 3. ИЗВЛЕЧЕНИЕ ИЗ OUTPUT_JSON
    -- ========================================================================
    
    v_payment_type := p_output_json->>'paymentType';
    v_resolution := p_output_json->>'resolution';
    v_has_bypass := p_output_json->>'hasBypass';
    
    -- Извлечение массива list_codes
    SELECT ARRAY(
        SELECT jsonb_array_elements_text(p_output_json->'listCodes')
    ) INTO v_list_codes;
    
    -- ========================================================================
    -- 4. ВСТАВКА В KSK_RESULT
    -- ========================================================================
    ...
    INSERT INTO upoa_ksk_reports.ksk_result (
        ...
        currency,
        currency_control,
        input_kafka_headers,    -- ДОБАВЛЕНО 28.10.2025
        output_kafka_headers    -- ДОБАВЛЕНО 28.10.2025
    ) VALUES (
        v_date,
        v_corr_id,
        p_input_timestamp,
        ...
        v_payer_inn,
        v_payer_name,
        v_payer_account_number,
        v_payer_document_type,
        v_payer_bank_name,
        v_payer_bank_account_number,
        v_receiver_account_number,
        v_receiver_name,
        v_receiver_inn,
        v_receiver_bank_name,
        v_receiver_bank_account_number,
        v_receiver_document_type,
        v_amount,
        v_currency,
        v_currency_control,
        p_input_kafka_headers,    -- ДОБАВЛЕНО 28.10.2025
        p_output_kafka_headers    -- ДОБАВЛЕНО 28.10.2025
    )
    RETURNING id INTO v_result_id;
    
    -- ========================================================================
    -- 5. ВСТАВКА ФИГУРАНТОВ (ЕСЛИ ЕСТЬ)
    -- ========================================================================
    
    v_figurants := p_output_json->'figurants';
    
    IF v_figurants IS NOT NULL AND jsonb_array_length(v_figurants) > 0 THEN
        FOR v_figurant IN SELECT * FROM jsonb_array_elements(v_figurants)
        LOOP
            -- Вставка записи в ksk_figurant
            INSERT INTO upoa_ksk_reports.ksk_figurant (
                ...
                v_result_id,
                v_figurant->>'listCode',
                v_figurant->>'nameFigurant',
                v_figurant->>'presidentGroup',
                v_figurant->>'autoLogin',
                v_figurant->>'hasExclusion',
                v_figurant->>'exclusionPhrase',
                v_figurant->>'isBypass'
            )
            RETURNING id INTO v_figurant_id;
            
            -- Вставка совпадений (matches) для фигуранта
            IF v_figurant->'matches' IS NOT NULL THEN
                ...
                SELECT
                    ...
                    m->>'matchPaymentField',
                    m->>'matchPaymentValue'
                FROM jsonb_array_elements(v_figurant->'matches') AS m;
            END IF;
        END LOOP;
    END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Ошибка в put_ksk_result: % (SQLSTATE: %)', SQLERRM, SQLSTATE;
END;
$$;

-- Комментарии
COMMENT ON FUNCTION upoa_ksk_reports.put_ksk_result(TIMESTAMP(3), TIMESTAMP(3), JSONB, JSONB, JSONB, JSONB) IS 
'Вставка записи результата проверки КСК с поддержкой Kafka headers. 
Обновлено: 28.10.2025 - добавлены параметры input_kafka_headers и output_kafka_headers';
...
-- Дата завершения: 2025-10-28 18:25:16
-- ============================================================================
